...
export default function CryptoSimulationPage() {
  ...
  useEffect(() => {
    async function fetchSimulationData() {
      try {
        let data: any = [];
        try {
          const resp = await fetch("/api/crypto-simulation", { cache: "no-store" });
          data = await resp.json();
        } catch (e) {}
        if (!Array.isArray(data) || data.length === 0) {
          const resp2 = await fetch("https://api.investorbabu.com/api/vps-data?file=crypto_simulated_orders");
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
        <div>
          <h1 className="text-4xl font-bold text-white tracking-tight font-display mb-2 flex items-center gap-3">
            Crypto Simulation <Badge variant="info" className="text-[0.6rem] uppercase tracking-widest px-2 py-0.5">Paper-Trading</Badge>
          </h1>
