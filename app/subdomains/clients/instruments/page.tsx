...
import { Plus, Loader2, X, Trash2, Edit2, Check, HelpCircle, Eye, EyeOff } from "lucide-react";
...
export default function ClientInstrumentsPage() {
  ...
  // Profile Settings for visibility toggles
  ...
  useEffect(() => {
    // Load toggle settings from localStorage if present
    const savedShowUs = localStorage.getItem("client_show_us") === "true";
    const savedShowCrypto = localStorage.getItem("client_show_crypto") === "true";
    setShowUs(savedShowUs);
    setShowCrypto(savedShowCrypto);

    fetchAllMarkets();
  }, []);

  const handleToggleUs = () => {
    const nextVal = !showUs;
    setShowUs(nextVal);
    localStorage.setItem("client_show_us", String(nextVal));
  };

  const handleToggleCrypto = () => {
    const nextVal = !showCrypto;
    setShowCrypto(nextVal);
    localStorage.setItem("client_show_crypto", String(nextVal));
  };
  ...
  const startEdit = (symbol: string) => {
    ...
    const modeConfig = itemConfig.live || { capital: 10000.0, lot_size: 0.0 };
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Scanned Instruments</h1>
          <p className="text-text-secondary">Manage and customize your live scanned instruments and sizing configurations.</p>
        </div>

        {/* Visibility Toggles / Profile Settings */}
        <div className="flex bg-white/5 border border-white/10 rounded-xl p-1 gap-2">
          <button
            onClick={handleToggleUs}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-wider flex items-center gap-1.5 ${showUs ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
          >
            {showUs ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
            US Commex
          </button>
          <button
            onClick={handleToggleCrypto}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-wider flex items-center gap-1.5 ${showCrypto ? 'bg-purple-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}
          >
            {showCrypto ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
            Crypto Market
          </button>
        </div>
      </div>
