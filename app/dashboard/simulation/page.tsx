...
export default function SimulationPage() {
  ...
  // Compute stats
  const activePositions = orders.filter(o => o.status === "ACTIVE");
  const completedPositions = orders.filter(o => ["TARGET HIT", "SL HIT"].includes(o.status));
  const pendingOrders = orders.filter(o => o.status === "PENDING");
  
  const totalPnL = orders.reduce((acc, o) => acc + (o.pnl || 0), 0);
  const targetHits = orders.filter(o => o.status === "TARGET HIT").length;
  const slHits = orders.filter(o => o.status === "SL HIT").length;
  
  const winRate = completedPositions.length > 0 
    ? (targetHits / completedPositions.length) * 100 
    : 0;

  const totalCapitalUsed = activePositions.reduce((acc, o) => {
    const qty = o.active_leg === "BUY" ? o.buy_qty : o.sell_qty;
    return acc + (o.entry_price * qty);
  }, 0);
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <GlassCard className="relative overflow-hidden p-6 border-white/5">
              ...
              <p className="text-xs text-slate-500">Combined realized and running unrealized</p>
              <div className={cn(
            ...
            {activePositions.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {activePositions.map((pos) => {
                  ...
                  const pct = Math.min(Math.max((dist / range) * 100, 0), 100);
                  ...
                  return (
                    <GlassCard key={pos.symbol + pos.time} className="p-6 border-white/5 relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-center text-[0.6rem] text-slate-500">
                        ...
                        <span>Target distance: {Math.abs(target - ltp).toFixed(2)} pts</span>
                      </div>
                    </GlassCard>
                  );
                })}
          ...
          <div>
            <h2 className="text-xl font-bold text-white mb-4 font-display flex items-center gap-2">
              Simulation Order Book <span className="text-xs text-slate-500 font-normal">({orders.length} orders total)</span>
            </h2>
                    ...
                    {orders.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="px-6 py-12 text-center text-slate-500 text-sm">
                          No simulated records for today. Waiting for TradingView signals...
                        </td>
                      </tr>
                    ) : (
                      orders.map((o, idx) => {
                        ...
                        return (
                          <tr key={idx} className="border-b border-white/5 hover:bg-white/[0.01] transition-colors font-mono">
                            ...
                            <td className="px-6 py-4 text-xs text-slate-400">
                              ...
                              <span className="text-emerald-500/80">{o.buy_target.toFixed(1)}</span>
                              <span className="text-slate-500 mx-1">/</span>
                            ...
                            <td className="px-6 py-4 text-xs text-slate-400">
                              ...
                              <span className="text-slate-500 mx-1">/</span>
                              <span className="text-emerald-500/80">{o.sell_target.toFixed(1)}</span>
                              <span className="text-slate-500 mx-1">/</span>
